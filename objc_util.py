...
def get_possible_method_names(name, args, kwargs):
  ...
  if len(args) == 0 and len(kwargs) == 0:
    ...
    for perm in permutations:
      ...
      if len(args) == 0:
        name2 = name + 'With%s_' % (_first_letter_cap(perm[0]), ) + '_'.join(
          perm[1:])
        if len(perm) > 1:
          name2 += '_'
...
def resolve_instance_method(obj, name, args, kwargs):
  ...
  for method_name, kwarg_order in possible_names:
    try:
      ...
      meth = ObjCInstanceMethod(obj,
                                method_name,
                                allow_property=possibly_property)
      valid_names.append((method_name, kwarg_order))
    except AttributeError:
      pass
  if len(valid_names) == 1:
...
def _add_method(method,
  ...
  if hasattr(method, 'encoding'):
    ...
    if superclass_method:
      ...
      for protocol_name in protocols:
        ...
        if protocol:
          ...
          method_desc = protocol_getMethodDescription(protocol, sel(sel_name),
                                                      False, True)
    ...
    if not type_encoding:
      ...
      type_encoding = 'v%i@0:8%s' % (sizeof(c_void_p) *
                                     (num_args + 2), ''.join(
                                       '@%i' % ((i + 2) * sizeof(c_void_p), )
